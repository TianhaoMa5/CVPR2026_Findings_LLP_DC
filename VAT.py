...
class VATLoss(nn.Module):
    ...
    def forward(self, model, x):
        with torch.no_grad():
            ...
            if logits.dim() == 1:  # 例如 [C]
                logits = logits.unsqueeze(0)  # -> [1, C]
            pred = F.softmax(logits, dim=-1)  # 总在最后一维做 softmax，兼容 [B, C] / [1, C]
...
def train_one_epoch(epoch,
    ...
    for it in range(len(dltrain_u)):
        ...
        if isinstance(ims_u_weak10, (tuple, list)):
            ims_u_weak10 = ims_u_weak10[0]  # 取第一个作为 logits
        else:
            ims_u_weak10 = ims_u_weak10
...
def main():
    ...
    dltrain_u ,length,_= get_train_loader(args.n_classes,
